...
func _physics_process(delta: float) -> void:
	...
	var active_gravity := gravity_override if gravity_override >= 0.0 else gravity
...
func _draw() -> void:
	var speed_squash := clampf(velocity.length() / max_speed, 0.0, 0.22)
	var aim_power := clampf(drag_vector.length() / max_drag_distance, 0.0, 1.0)
	var stretch := Vector2(1.0 + speed_squash * 0.5, 1.0 - speed_squash * 0.32)
	if motion_state == MotionState.AIMING and drag_vector.length() > 1.0:
		stretch = Vector2(1.0 + aim_power * 0.38, 1.0 - aim_power * 0.22)
	...
	draw_circle(Vector2.ZERO, radius, skin_color)
	draw_arc(Vector2.ZERO, radius, 0.0, TAU, 56, outline, 5.0)
	...
	if motion_state == MotionState.AIMING:
		...
		draw_arc(Vector2.ZERO, radius + 12.0, 0.0, TAU, 60, Color(0.96, 1.0, 0.74, 0.5), 4.0)
